//
//  BusinessDashboardViewController.m
//  Panachz
//
...
@interface BusinessDashboardViewController () <SlideNavigationControllerDelegate, UITableViewDataSource, UITableViewDelegate, PNChartDelegate, UIGestureRecognizerDelegate>
...
- (void)viewDidLoad {
    ...
    NSString *titleText = @"Business Dashboard";
...
- (void)getThisMonthRevenue {
    ...
    [[PanachzApi getInstance] GET:[NSString stringWithFormat:@"user/%lu/orders/month", [Panachz getInstance].user.userId]
                       parameters:nil
...
- (void)getTodayRevenue {
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view
                                              animated:YES];
    __weak typeof (self) weakSelf = self;
    [[PanachzApi getInstance] GET:[NSString stringWithFormat:@"user/%lu/orders/today", [Panachz getInstance].user.userId]
                       parameters:nil
                          success:^(NSURLSessionDataTask *task, id responseObject) {
    ...
    MBProgressHUD *hud2 = [MBProgressHUD showHUDAddedTo:self.view
                                              animated:YES];
    [[PanachzApi getInstance] GET:[NSString stringWithFormat:@"user/%lu/orders/today/increase", [Panachz getInstance].user.userId]
                       parameters:nil
                          success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)getYesterdayRevenue {
    ...
    MBProgressHUD *hud2 = [MBProgressHUD showHUDAddedTo:self.view
                                              animated:YES];
    [[PanachzApi getInstance] GET:[NSString stringWithFormat:@"user/%lu/orders/7/increase", [Panachz getInstance].user.userId]
                       parameters:nil
                          success:^(NSURLSessionDataTask *task, id responseObject) {
                              CGFloat revenuePercentage = [[responseObject objectForKey:@"revenue"] floatValue];
                              CGFloat orderPercentage = [[responseObject objectForKey:@"order"] floatValue];
...
- (void)showStatisticsForLast:(NSUInteger)days {
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view
                                              animated:YES];
    __weak typeof (self) weakSelf = self;
    [[PanachzApi getInstance] GET:[NSString stringWithFormat:@"user/%lu/orders/%lu/stat", [Panachz getInstance].user.userId, days]
                       parameters:nil
                          success:^(NSURLSessionDataTask *task, id responseObject) {
                              ...
                              if ([[response objectForKey:@"code"] intValue] == 100 ) {
                                  ...
                                  for (NSDictionary *stat in statArray) {
